...
%**************************************************************************
%Inserimento dei PARAMETRI FISSI, quelli che restano cioè costanti in
...
initialMaxLength=4;                 %Lunghezza massima delle specie da creare
virtual_fd_max_length = 4;          %Lunghezza massima fino alla quale creare le reazioni
maxLOut = 0;						%se=0 non viene considerato, altrimenti... Quando influx_rate > 0 indica la lunghezza massima delle molecole che possono uscire dal contenitore, quando influx_rate = 0 indica fino a quale lunghezza le molecole non variano in quantita' (simulazione membrana permeabile)
ECConcentration=0;
alphabet = ['AB'];
...
reverseReactions=0;
reactionProbability=1.033060e-03;
Kass = 50;
Kdiss = 25;
Kcpx = 50;
K_cpx = 1;
...
revRctRatio = 10;
ratioSpeciesEnergizable = 0;         %percentuale di specie presenti nel sistema che possono essere energizzate  per ogni specie create c'è una certa probabilità di essere energizzabile o meno
moleculeDecay_KineticConstant=0.02;
diffusion_contribute=0;
solubility_threshold=0;
influx_rate=1e-21;                       %se è 0 il sistema è chiuso
...
lunghezza_max_fd = 4;                %lunghezza dei monomeri/polimeri da conservare
...
controllo_ACS_nel_ciclo = 4;         %controllo che non ci siano cicli nell'influx %1: nessun controllo %2: no cicli nel grafo substrato-prodotto %3: no cicli nel grafo catalizzatore-prodotto 4: no cicli nel grafo catalizzatore-prodotto nè nel grafo substrato-prodotto
                                     % NOVITA': se metto
                                     % controllo_ACS_nel_ciclo = 5 accetto
                                     % solo quelle reti che hanno
                                     % necessariamnete un ciclo nel flusso.

simFolder.nets = 8;					% Number of different networks ensambles, for every network will be performed nSim different simulation (differnt random seeds)
simFolder.name = '0_CSTR_attractors';       %Nome della cartella dove verrˆ salvata la simulazione
simFolder.path = '~/Documents/init/';   %Percorso dove verrˆ creata la cartella simFolder dove verranno salvati tutti i file
...
%carichi per rendere possibile la reazione. Nello specifico la tabella è
%questa:
%C	S1	S2
%+	+	+
%+	+	-
...
%un esempio di funzione  booleana sarà pertanto 01100101
...
%assegnate con probabilità uniforme alle reazioni 
%la funzione è divisa a metà, la prima metà corrisponde alla condensazione
...
for i=1:length(funzioni_booleane_in_dec) %assegno la probabilità di scelta della funzione booleana, per default è uniforme
...
for i = 1:b %numero di valori delle variabili (e.g. numero di valori della probabilità)
    
        for z = 1:simFolder.nets %numero di reti per ogni combinazione di variabili
    
            %scrivo il nome delle cartelle
            parte1_nome_cartella = ('CSTR_');
            ...
            riga_lanciatore_2 = strcat('echo "Reverse/',nome_cartella,'" ;',riga_lanciatore);
            count = fprintf(fid20,'%s\n ',riga_lanciatore_2);
            clear riga_lanciatore
                                          
            %lancio il vero e proprio inizializzatore            
            inizializzatore_ACS(nGEN, nSIM, nSeconds, nReactions, initialMaxLength, virtual_fd_max_length, overallConcentration, ...
                ...
                randomSeed, debugLevel, timeStructuresSavingInterval,maxLOut,simFolder,lMaxInflux(i),fileTimesSaveInterval,nHours,nAttempts,revRctRatio);
