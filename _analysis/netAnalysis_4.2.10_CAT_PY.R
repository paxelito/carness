...
allNETgss <- function(tmpParInflux=0,tmpFolderResSubName="sim", tmpGraphFolderSuffix="", tmpTotGens=1, tmpTotSims=1, maxRctN=200000000)
{
	...
	for(tmpf in 1 : length(simFolds))
	{
		...
		gssNET(1, tmpTotGens, 1, tmpTotSims, maxRctN, tmpGraphFolderSuffix, tmpSimPath, length(simFolds), tmpFolderResSubName, tmpParInflux, tmpf);
	}
...
gssNET <- function(tmpGen, tmpTotGens, tmpSim, tmpTotSims, tmpMaxRctN, tmp___GraphFolderSuffix, tmpSimPath, tmpNsimFolds,tmpFolderResSubName, tmpParInflux, tmpf)
{
	...
	{	
		...
		rctParFileList		<- dir(pattern = "reactions_parameters_");			
		livSpeciesFileList	<- dir(pattern = "livingSpecies_");
		livAmountFileList	<- dir(pattern = "livingAmount_");
		livConcFileList		<- dir(pattern = "livingConcentration_");
		...
		setwd(paste("../../__0_iGraph_CAT", tmp___GraphFolderSuffix, sep=""))
			...
			#iGraphNetFile <- dir(pattern = paste("_iGraph_CAT", print(zeroBeforeStringNumber(tmpf, maxNumberOfSimulation * maxNumberOfSimulation));
			strZerosFolders <- zeroBeforeStringNumber(maxNumberOfSimulation * maxNumberOfSimulation, tmpf);
			#print(paste("_iGraph_CAT_", strZerosFolders, tmpf, "_", sep=""));
			iGraphNetFile <- dir(pattern = paste("_iGraph_CAT_", strZerosFolders, tmpf, "_", sep=""));
			subStrGraph = paste("_", strZerosFolders, tmpf, sep="");
			...
			for(iGraphFile in 1 : length(iGraphNetFile))
			{
			 ...
			 # -----------------------------------------------    					 	    
				...
				# If there are strongly connected components							
			 	    if(N_SCC_cat_LT > 0)
			 	    {
			 	    	css_ids <- which(SCC_cat_LT$csize > 1);
			 	    	#print(iGraphAdjCatMatrix_LIFETIME)
			 	    	#print(SCC_cat_LT)
			 	    	#print(css_ids)
			 	    	...
			 	    	nLoop <- NULL;
			 	    	if(length(loopSpecies) > 0) #LOOP 
			 	    	{
				 	    	for(loopSpeciesID in 1 : length(loopSpecies))
				 	    	{
				 	    		nLoop <- c(nLoop, which(V(iGraphAdjCatMatrix_LIFETIME)$name == loopSpecies[loopSpeciesID]))
				 	    	}
			 	    	}
					for(i in 1 : N_SCC_cat_LT) # ACS
					{
					...
					tot_SCC_speciesVTX_cat_LT <- V(iGraphAdjCatMatrix_LIFETIME)[tot_SCC_speciesID_cat_LT - 1];
					#print(V(iGraphAdjCatMatrix_LIFETIME))
					IDspeciesPresentInScc = as.integer(tot_SCC_speciesVTX_cat_LT$name);

					#print(V(iGraphAdjCatMatrix_LIFETIME))
					#print(css_ids[i])
					#print(sng_SCC_speciesID_cat_LT)
					#print(tot_SCC_speciesVTX_cat_LT)
					#print(IDspeciesPresentInScc)
					#readline(prompt = "Pause. Press <Enter> to continue…");
...
#				    readline(prompt = "Pause. Press <Enter> to continue...")
				...
				setwd(paste("./__02_nets", tmp___GraphFolderSuffix, sep=""));
					if(N_SCC_cat_LT > 0) # NO LIFETIME
					{
						...
						graphCompleteWeightNetONLYSCCs(onlySCCgraph, netImageName, 0, influxIds, livingSpecies, livingAmount, tot_SCC_speciesVTX_cat_LT)												
						#readline(prompt = "Pause. Press <Enter> to continue...")
					}else{
